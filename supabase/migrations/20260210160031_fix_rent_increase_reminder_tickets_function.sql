/*
  # Fix rent increase reminder tickets function

  1. Changes
    - Fixed column names: `title` -> `subject`, removed `description` (column doesn't exist)
    - Fixed reference to non-existent `next_rent_increase_date` - now calculates from
      `index_first_increase_date` (for index rent) and `graduated_rent_date` (for staffel rent)
    - Added `user_id` and `ticket_type` which are required by tickets table
    - Fixed duplicate-check to use `subject` instead of `title`
    - Added `auto_create_rent_increase_tickets` filter
    - Creates a ticket_message with the description text after inserting the ticket

  2. Notes
    - The old function referenced columns that don't exist in the tickets table,
      causing the cron job to fail every run
    - ticket_number is auto-generated by trigger so does not need to be specified
*/
