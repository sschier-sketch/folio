/*
  # Create LetterXpress job sync cron and update admin system health

  1. New Cron Job
    - `sync-letterxpress-jobs` - runs every 30 minutes
    - Calls the sync-letterxpress-jobs edge function
    - Iterates all enabled LetterXpress accounts
    - Fetches printjobs from LetterXpress API and upserts into local mirror
    - Respects rate limits (120 req/min) with 600ms delay between accounts

  2. Updated Functions
    - `admin_get_system_health()` - Added sync-letterxpress-jobs to job_meta
      with German description for Admin -> Cronjobs visibility

  3. Notes
    - The cron uses service_role_key for auth (no user context)
    - Per-account errors do not abort the entire run
    - is_cancelable is recalculated on every sync based on status + 15-min window
*/
...
BEGIN
  ...
  PERFORM cron.schedule(
    ...
    SELECT net.http_post(
      url := (SELECT decrypted_secret FROM vault.decrypted_secrets WHERE name = 'SUPABASE_URL' LIMIT 1) || '/functions/v1/sync-letterxpress-jobs',
...
-- 2. Update admin_get_system_health to include the new cron job
