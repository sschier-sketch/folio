/*
  # Fix index rent calculation to be user-scoped

  The run_automatic_index_rent_calculations function was processing ALL users'
  contracts globally, causing incorrect contract counts. Now it only processes
  contracts belonging to the calling user via auth.uid().

  1. Changes
    - Add WHERE rc.user_id = auth.uid() filter to the contract loop
    - This ensures each user only sees their own contracts counted
*/
