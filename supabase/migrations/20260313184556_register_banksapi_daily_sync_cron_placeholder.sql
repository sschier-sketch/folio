/*
  # Register BanksAPI Daily Sync Cron Placeholder

  1. Changes
    - Creates a placeholder cron job `banksapi-daily-sync` that runs daily at 05:00 UTC
    - Calls a simple SELECT 1 as placeholder (actual sync logic to be added later)
    - Updates admin_get_system_health to include this job in its description list
    - Job is visible in Admin -> Cronjobs

  2. Notes
    - This is foundation-only: actual sync logic will be implemented later
    - The job currently executes a no-op query
*/

-- Create the placeholder cron job
...
-- Update admin_get_system_health to include the new job description
...
('banksapi-daily-sync', 'Synchronisiert Bankumsaetze via BanksAPI PSD2-Schnittstelle (Platzhalter - noch nicht aktiv)', 'pg_cron')
),
