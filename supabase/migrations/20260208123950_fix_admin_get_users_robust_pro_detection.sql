/*
  # Make admin_get_users more robust for Pro detection

  1. Changes
    - Removes `deleted_at IS NULL` filters on Stripe JOINs so admin always sees 
      the full picture regardless of soft-delete state
    - Adds fallback: if `billing_info.subscription_ends_at` is in the future, 
      the user is still Pro (covers edge cases where Stripe JOIN fails)
    - Handles 'trialing' and 'past_due' statuses as still Pro
    - Returns `cancel_at_period_end` and `stripe_status` for full admin visibility

  2. Why This Is Needed
    - Some users' Stripe records may have `deleted_at` set, causing the JOIN to 
      silently exclude them
    - The `subscription_ends_at` field is set when a user cancels but is still 
      in their paid period -- this is a strong signal the user is still Pro
*/
