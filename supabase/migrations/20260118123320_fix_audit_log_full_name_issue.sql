/*
  # Fix Audit Log Trigger - full_name Issue
  
  1. Problem
    - Die Funktion `log_property_change()` versucht `full_name` aus `account_profiles` zu lesen
    - Diese Spalte existiert nicht - stattdessen gibt es `first_name` und `last_name`
    - Dies verhindert das Löschen von Immobilien
  
  2. Lösung
    - Funktion korrigieren um `first_name || ' ' || last_name` zu verwenden
    - Fallback auf email wenn Name nicht vorhanden ist
    - Robuster machen gegen fehlende Daten
*/
...
BEGIN
  ...
  -- Get username from account_profiles (KORRIGIERT: first_name + last_name statt full_name)
  SELECT COALESCE(
  ...
  -- Insert audit log entry (mit Exception Handling für mehr Stabilität)
  BEGIN
