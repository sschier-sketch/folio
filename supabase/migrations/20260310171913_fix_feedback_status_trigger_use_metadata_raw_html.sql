/*
  # Fix feedback status trigger to use metadata->raw_html instead of body_html/body_text

  1. Bug Fix
    - The `notify_user_on_feedback_status_change()` was trying to INSERT into
      non-existent columns `body_html` and `body_text` on `email_logs`
    - The email system stores rendered HTML in `metadata->raw_html` with `send_raw = true`
    - Updated to match the pattern used by `notify_admin_on_new_registration()`

  2. Changes
    - Render the template into raw HTML and store in metadata->raw_html
    - Remove references to body_html and body_text columns
    - Use metadata with send_raw pattern for email queue processing
*/
