/*
  # Fix index rent basis_monat to use rent_history effective_date

  1. Changes
    - The basis_monat was previously derived from MAX(calculation_date) of prior
      index_rent_calculations or from contract_start_date. This led to incorrect
      base months (e.g., showing 2024-01 when the user had retroactively set a
      rent change to 2025-05-01).
    - Now basis_monat is derived from the most recent rent_history entry with
      reason='index' that has VPI values (i.e., an actual index increase done
      through the system). If no such entry exists, basis_monat is set to NULL,
      so the UI can display "Unbekannt".
    - The possible_since calculation now uses the latest effective_date from
      rent_history (any reason) to determine the 12-month waiting period,
      ensuring retroactive rent changes are respected.
    - The last_calculation_date check (for skipping contracts already calculated)
      remains unchanged.

  2. Important
    - No existing data is modified
    - Only the function logic changes
*/
  ...
  LOOP
    ...
    IF v_months_since_last >= 12 THEN
      ...
      INSERT INTO mail_messages (
        ...
        CASE WHEN v_basis_month IS NOT NULL THEN 'Basismonat: ' || v_basis_month || chr(10) ELSE '' END || chr(10) ||
