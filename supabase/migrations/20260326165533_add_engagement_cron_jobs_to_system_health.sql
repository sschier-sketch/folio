/*
  # Add engagement email cron jobs to admin system health

  1. Changes
    - Activates both engagement email cron jobs (daily-inactive-user-with-property, daily-no-property-reminder)
    - Adds both jobs to the `admin_get_system_health()` function so they
      appear in the admin monitoring dashboard

  2. Cron Jobs
    - `daily-inactive-user-with-property` -- daily at 08:00 UTC, sends re-engagement email to users with properties who haven't logged in for 17+ days
    - `daily-no-property-reminder` -- daily at 08:15 UTC, sends reminder to users who registered 17 days ago but never created a property
*/

-- 1. Ensure both jobs are active
...
-- 2. Update admin_get_system_health to include the two engagement cron jobs
...
('daily-inactive-user-with-property','pg_cron',       'Sendet taeglich Re-Engagement-E-Mails an Nutzer mit Immobilien die seit 17+ Tagen inaktiv sind.'),
('daily-no-property-reminder',       'pg_cron',       'Sendet taeglich Erinnerungen an Nutzer die vor 17 Tagen registriert aber keine Immobilie angelegt haben.')
),
cron_jobs AS (
SELECT
j.jobname,
