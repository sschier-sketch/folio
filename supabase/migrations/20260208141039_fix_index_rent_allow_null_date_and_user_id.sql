/*
  # Fix index rent calculation - allow null first increase date + include user_id

  1. Changes
    - Removed the requirement for index_first_increase_date IS NOT NULL in WHERE clause
    - When index_first_increase_date is NULL, defaults to contract_start + 12 months
    - Includes user_id in INSERT into index_rent_calculations (fixes not-null constraint)

  2. Notes
    - Contracts like "Florian Esterl" that have rent_increase_type = 'index' but no
      index_first_increase_date were being silently excluded from calculations
    - The 12-month default aligns with German law (minimum interval for index rent)
*/
