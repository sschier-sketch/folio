/*
  # Update admin_get_system_health with all cron job descriptions

  1. Modified Functions
    - `admin_get_system_health()` - Updated job_meta CTE to include descriptions
      for ALL 9 active cron jobs:
      - daily-activate-planned-rent-periods (NEW)
      - daily-analytics-snapshot (was missing)
      - cleanup-bank-import-files-daily (was missing)
      - sync-stripe-invoices-daily (was missing)
      - daily-index-rent-calculations (existing)
      - daily-rent-increase-reminders (existing)
      - process-email-queue (existing)
      - daily-sitemap-generation (existing)
      - weekly-interest-rate-fetch (existing)

  2. Data Fix
    - Resets orphaned parking unit (Bernauer Strasse) from 'rented' to 'vacant'
    - The unit had no rental_contract_units link but status was stuck at 'rented'

  3. Important Notes
    - No schema changes, only function replacement and one data fix
*/

-- 1. Fix orphaned parking unit status
-- Must disable and re-enable trigger to bypass prevent_manual_rented_status
ALTER TABLE property_units DISABLE TRIGGER trigger_prevent_manual_rented;
...
ALTER TABLE property_units ENABLE TRIGGER trigger_prevent_manual_rented;

-- 2. Update admin_get_system_health with all job descriptions
