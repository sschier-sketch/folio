/*
  # Add monthly feature count cache to system_settings

  1. Modified Tables
    - `system_settings`
      - `monthly_feature_count` (integer, default 0) - cached count of published
        system_updates in the current month, updated daily by cron

  2. New Functions
    - `update_monthly_feature_count()` - counts published system_updates where
      published_at falls in the current calendar month, writes result to
      system_settings.monthly_feature_count

  3. Updated Functions
    - `get_system_settings()` - now returns `monthly_feature_count`
    - `admin_get_system_health()` - includes the new cron job
      `daily-update-monthly-feature-count` in its job_meta list

  4. New Cron Job
    - `daily-update-monthly-feature-count` - runs daily at 00:05 UTC,
      refreshes the cached monthly feature count

  5. Important Notes
    - The count is based on system_updates rows with is_published = true
      AND published_at >= first day of the current month
    - An initial run is triggered immediately so the value is populated
*/
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE table_name = 'system_settings' AND column_name = 'monthly_feature_count'
  ) THEN
    ALTER TABLE system_settings ADD COLUMN monthly_feature_count integer NOT NULL DEFAULT 0;
  END IF;
END $$;

-- 2. Create the update function
CREATE OR REPLACE FUNCTION public.update_monthly_feature_count()
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
-- 4. Schedule daily cron at 00:05 UTC
SELECT cron.schedule(
  'daily-update-monthly-feature-count',
  '5 0 * * *',
  $$SELECT public.update_monthly_feature_count()$$
);

-- 5. Update get_system_settings to include monthly_feature_count
...
-- 6. Update admin_get_system_health to include the new cron job
