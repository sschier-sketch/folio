/*
  # Fix LetterXpress duplicate key constraint and add queue processing cron

  1. Modified Tables
    - `letterxpress_jobs`
      - `external_job_id` changed from NOT NULL integer to nullable integer
      - Old unique constraint `letterxpress_jobs_owner_extid_unique` replaced with
        a partial unique index that only enforces uniqueness when external_job_id IS NOT NULL
      - This allows multiple pending/queued jobs per user (which have external_job_id = NULL)

  2. New Cron Job
    - `process-letterxpress-queue` - runs every 2 minutes
    - Calls the process-letterxpress-queue edge function
    - Picks up pending jobs and submits them to LetterXpress
    - Acts as a safety net in case the immediate trigger via EdgeRuntime.waitUntil fails

  3. Updated Functions
    - `admin_get_system_health()` - Added process-letterxpress-queue to job_meta

  4. Notes
    - Existing rows with external_job_id = 0 are updated to NULL if status is pending/error
    - The sync upsert still works because real jobs from LetterXpress always have non-null IDs
*/

-- Step 1: Drop the old unique constraint
ALTER TABLE letterxpress_jobs
  DROP CONSTRAINT IF EXISTS letterxpress_jobs_owner_extid_unique;

-- Step 2: Allow NULL for external_job_id
ALTER TABLE letterxpress_jobs
  ALTER COLUMN external_job_id DROP NOT NULL;

-- Step 3: Clean up placeholder zeros from queued jobs
UPDATE letterxpress_jobs
SET external_job_id = NULL
WHERE external_job_id = 0
  AND status IN ('pending', 'error', 'processing');

-- Step 4: Create a partial unique index (only for real external IDs)
CREATE UNIQUE INDEX IF NOT EXISTS idx_letterxpress_jobs_owner_extid_unique
  ON letterxpress_jobs (user_id, external_job_id)
  WHERE external_job_id IS NOT NULL;

-- Step 5: Schedule the queue processor cron job
DO $outer$
...
BEGIN
  ...
  PERFORM cron.schedule(
    ...
    SELECT net.http_post(
      url := (SELECT decrypted_secret FROM vault.decrypted_secrets WHERE name = 'SUPABASE_URL' LIMIT 1) || '/functions/v1/process-letterxpress-queue',
...
-- Step 6: Update admin_get_system_health to include the queue processor
