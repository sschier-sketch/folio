/*
  # Create trial email cron jobs and fix admin cron list

  1. New Functions
    - `trigger_cron_trial_ending()` - calls the cron-trial-ending Edge Function
      via pg_net to send "trial ending soon" emails to users whose trial
      expires in 7 days
    - `trigger_cron_trial_ended()` - calls the cron-trial-ended Edge Function
      via pg_net to send "trial ended" emails to users whose trial has expired

  2. New Cron Jobs
    - `daily-trial-ending-emails` - runs daily at 07:00 UTC (08:00 MEZ)
    - `daily-trial-ended-emails` - runs daily at 07:30 UTC (08:30 MEZ)

  3. Modified Functions
    - `admin_get_cron_jobs()` - updated to prevent duplicate entries when
      an edge function has both a pg_cron job AND cron_runs entries

  4. Important Notes
    - These two edge functions existed since the trial system was built but
      were NEVER scheduled -- no cron job was ever created for them
    - The CRON_SECRET auth check was removed from both functions (deployed
      with verifyJWT=false) since pg_net cannot pass custom secrets
    - Both functions have idempotency protection so duplicate runs are safe
    - The "trial ending" job runs 30 minutes before "trial ended" to avoid
      sending both emails on the same morning
*/

-- 1. Trigger function for cron-trial-ending
CREATE OR REPLACE FUNCTION public.trigger_cron_trial_ending()
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  PERFORM net.http_post(
    url := v_supabase_url || '/functions/v1/cron-trial-ending',
    headers := v_headers,
    body := '{}'::jsonb
  );
END;
...
-- 2. Trigger function for cron-trial-ended
CREATE OR REPLACE FUNCTION public.trigger_cron_trial_ended()
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  PERFORM net.http_post(
    url := v_supabase_url || '/functions/v1/cron-trial-ended',
    headers := v_headers,
    body := '{}'::jsonb
  );
END;
...
BEGIN
  SELECT EXISTS (
    SELECT 1 FROM cron.job WHERE jobname = 'daily-trial-ending-emails'
  ) INTO v_job_exists;

  IF v_job_exists THEN
    PERFORM cron.unschedule('daily-trial-ending-emails');
  END IF;

  PERFORM cron.schedule(
    'daily-trial-ending-emails',
    '0 7 * * *',
    $$SELECT public.trigger_cron_trial_ending()$$
  );
END $outer$;
...
BEGIN
  SELECT EXISTS (
    SELECT 1 FROM cron.job WHERE jobname = 'daily-trial-ended-emails'
  ) INTO v_job_exists;

  IF v_job_exists THEN
    PERFORM cron.unschedule('daily-trial-ended-emails');
  END IF;

  PERFORM cron.schedule(
    'daily-trial-ended-emails',
    '30 7 * * *',
    $$SELECT public.trigger_cron_trial_ended()$$
  );
END $outer$;

-- 4. Update admin_get_cron_jobs to prevent duplicate entries
-- Edge functions that log to cron_runs AND have a pg_cron job would
-- otherwise appear twice. Filter out ef_result entries whose job_name
-- corresponds to a known pg_cron-triggered edge function.
...
BEGIN
  ...
  -- Edge function names that are triggered by pg_cron (via trigger functions)
  -- These should NOT appear in the ef_result section to avoid duplicates
  ...
  ef_result AS (
    SELECT
      ...
      (CASE efn.job_name
        ...
        WHEN 'create-rent-increase-reminders' THEN 'extern (taeglich)'
        WHEN 'run-index-rent-calculations' THEN 'extern (taeglich)'
        ELSE 'extern'
