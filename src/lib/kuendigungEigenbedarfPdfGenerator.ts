...
import type {
  ...
  TenantEntry,
  GreetingData,
  EigenbedarfSachverhalt,
} from '../components/wizard-templates/types';

interface EigenbedarfPdfInput {
...
function buildPropertyInline(t: TenantEntry): string {
  return `${t.street} ${t.number}, ${t.zip} ${t.city}`.trim();
}
...
export function generateEigenbedarfPdf(input: EigenbedarfPdfInput): Blob {
  ...
  doc.setFontSize(11);
  doc.setFont('helvetica', 'bold');
  doc.text('Kündigung wegen Eigenbedarf', ML, y);
  ...
  const propertyInline = buildPropertyInline(primaryTenant);
  writeLines(
    `Hiermit kündige ich wegen Eigenbedarfs gemäß §§ 573 Abs. 1, 573 Abs. 2 Nr. 2 BGB das Mietverhältnis über die im ${propertyInline} gelegene Wohnung fristgemäß zum ${formatDate(sachverhalt.raeumungsdatum)}, hilfsweise zum nächstmöglichen Zeitpunkt.`,
  );
  ...
  y += 2;
  writeLines('Der von mir/ uns geltend gemachte Eigenbedarf ergibt sich aus den nachfolgenden Umständen:');

  y += 3;
  if (sachverhalt.raeumungsgrund) {
    writeLines(sachverhalt.raeumungsgrund);
  }

  y += 4;
  writeLines(
    'Ich bitte Sie, mit mir einen Termin für die Übergabe zu vereinbaren, um die ordnungsgemäße Übergabe sicherzustellen.',
  );
...
export function buildEigenbedarfFilename(tenants: TenantEntry[]): string {
  ...
  const einheit = (t?.unitNumber || '')
    ...
    .replace(/_+/g, '_')
    .replace(/^_|_$/g, '');
  const parts = ['Kuendigung_Eigenbedarf', String(year)];
