...
import type {
  ...
  TenantEntry,
  GreetingData,
  KuendigungAbmahnungSachverhalt,
} from '../components/wizard-templates/types';

interface KuendigungAbmahnungPdfInput {
...
export function generateKuendigungAbmahnungPdf(input: KuendigungAbmahnungPdfInput): Blob {
  ...
  doc.setFontSize(11);
  doc.setFont('helvetica', 'bold');
  doc.text('Fristlose Kündigung nach Abmahnung', ML, y);
  ...
  const warnungsgrund = sachverhalt.warnungsgrund || '[Abmahnungsgrund]';
  const abmahnungText = `Mit Schreiben vom ${formatDate(sachverhalt.datumWarnung)} hatte ich Sie wegen ${warnungsgrund} abgemahnt und Sie aufgefordert, die Vertragsverletzungen zu unterlassen. Ich hatte Ihnen eine Frist bis zum ${formatDate(sachverhalt.fristBis)} gesetzt.`;
  writeLines(abmahnungText);

  y += 4;
  writeLines('Leider musste ich feststellen, dass Sie meine oben genannte Abmahnung nicht zum Anlass genommen haben, die Vertragsverletzungen abzustellen. Insbesondere haben Sie:');

  y += 3;
  if (sachverhalt.kuendigungsgrund) {
    writeLines(sachverhalt.kuendigungsgrund);
  }

  y += 4;
  writeLines(`Auf Grund der weiterhin bestehenden Vertragsverletzung kündige ich das Mietverhältnis gemäß § 543 BGB fristlos, hilfsweise fristgemäß, und fordere Sie auf, das Mietobjekt zu räumen und geräumt spätestens bis zum ${formatDate(sachverhalt.verlassenBis)} zurückzugeben.`);

  y += 4;
  writeLines(`Höchst vorsorglich weise ich Sie daraufhin, dass Sie einer ordentlichen Kündigung gemäß § 574 BGB widersprechen und die Fortsetzung des Mietverhältnisses verlangen können. Dies setzt voraus, dass die Beendigung des Mietverhältnisses für Sie oder Ihre Familie eine Härte bedeuten würde. Der Widerspruch bedarf der Schriftform. Er muss bis zum ${formatDate(sachverhalt.berufungsfrist)} bei mir eingehen. Ich fordere Sie auf, den Widerspruch zu begründen. Ich merke allerdings an, dass ein Widerspruchsrecht gemäß § 574 Abs. 1 S. 2 BGB dann nicht gegeben ist, wenn ein Grund vorliegt, der den Vermieter zur außerordentlichen fristlosen Kündigung berechtigt.`);

  y += 4;
  writeLines('Ich bitte Sie, mit mir einen Termin für die ordnungsgemäße Übergabe zu vereinbaren.');
  ...
  y += 4;
  writeLines('Bis zur Rückgabe des Mietobjekts bleiben Sie zur Zahlung einer Nutzungsentschädigung, zumindest in Höhe der geschuldeten Miete, verpflichtet.');

  y += 4;
  writeLines('Sofern Sie das Mietobjekt nicht freiwillig zurückgeben, werde ich umgehend Räumungs- und Zahlungsklage gegen Sie einreichen. Hierdurch entstehen weitere erhebliche Gerichts-, Rechtsanwalts- und Gerichtsvollzieherkosten, die durch Sie zu tragen wären.');
...
export function buildKuendigungAbmahnungFilename(tenants: TenantEntry[]): string {
  ...
  const einheit = (t?.unitNumber || '')
    ...
    .replace(/_+/g, '_')
    .replace(/^_|_$/g, '');
  const parts = ['Kuendigung_nach_Abmahnung', String(year)];
