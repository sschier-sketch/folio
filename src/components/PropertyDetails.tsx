...
import { ArrowLeft, Plus, TrendingUp, CreditCard, Users, Edit2, Trash2, Info } from 'lucide-react';
...
export default function PropertyDetails({ property, onBack }: PropertyDetailsProps) {
  const { user } = useAuth();
  const [currentView, setCurrentView] = useState<'overview' | 'statistics'>('overview');
  ...
  const loadData = async () => {
    ...
    try {
      const [loansRes, contractsRes] = await Promise.all([
        supabase.from('loans').select('*').eq('property_id', property.id).order('created_at', { ascending: false }),
        supabase.from('rental_contracts').select('*').eq('property_id', property.id).order('created_at', { ascending: false }),
      ]);
  ...
  const calculateYield = () => {
    ...
    const totalLoanPayments = loans.reduce((sum, l) => sum + Number(l.monthly_payment) * 12, 0);
  ...
  return (
    <div>
          ...
          {showGrossYieldTooltip && (
            <div className="absolute top-full left-0 mt-2 w-80 bg-dark text-white text-sm rounded-full p-4 z-10">
              ...
              <div className="space-y-1 text-xs">
                ...
                <div>Monatliche Kaltmiete: {formatCurrency(contracts.reduce((sum, c) => sum + Number(c.base_rent), 0))}</div>
                <div>Jahresmiete: {formatCurrency(contracts.reduce((sum, c) => sum + Number(c.base_rent), 0) * 12)}</div>
          ...
          {showNetYieldTooltip && (
            <div className="absolute top-full left-0 mt-2 w-80 bg-dark text-white text-sm rounded-full p-4 z-10">
              ...
              <div className="space-y-1 text-xs">
                ...
                <div className="border-t border-slate-700 my-2"></div>
                <div>Jahresmiete: {formatCurrency(contracts.reduce((sum, c) => sum + Number(c.base_rent), 0) * 12)}</div>
                <div>Jährliche Kreditkosten: {formatCurrency(loans.reduce((sum, l) => sum + Number(l.monthly_payment) * 12, 0))}</div>
                <div>Netto-Jahreseinkommen: {formatCurrency(contracts.reduce((sum, c) => sum + Number(c.base_rent), 0) * 12 - loans.reduce((sum, l) => sum + Number(l.monthly_payment) * 12, 0))}</div>
                <div>Aktueller Wert: {formatCurrency(property.current_value)}</div>
                <div className="border-t border-slate-700 my-2"></div>
                <div className="font-semibold">= {calculateYield().toFixed(2)}%</div>
              </div>
            </div>
          )}
        </div>
      ...
      {currentView === 'statistics' && (
        <PropertyStatistics property={property} onClose={() => setCurrentView('overview')} />
      )}
