...
export default function RentalContractModal({ contract, properties, onClose, onSave }: RentalContractModalProps) {
  const { user } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const contractData = {
        ...
        staffel_amount: formData.rent_increase_type === 'staffel' ? Number(formData.staffel_amount) : null,
        staffel_type: formData.rent_increase_type === 'staffel' ? formData.staffel_type : null,
        staffel_years: formData.rent_increase_type === 'staffel' ? Number(formData.staffel_years) : null,
        index_first_increase_date: formData.rent_increase_type === 'index' ? formData.index_first_increase_date || null : null,
        auto_create_rent_increase_tickets: formData.rent_increase_type !== 'none' ? formData.auto_create_rent_increase_tickets : false,
      ...
      if (contract) {
        ...
        const existingTenantIds = contract.tenants?.map(t => t.id).filter(Boolean) || [];
