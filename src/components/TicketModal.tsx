...
export default function TicketModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (ticket) {
        // Don't update ticket_number on edit const { error } = await supabase .from('tickets') .update(ticketData) .eq('id', ticket.id);
        if (error) throw error;
                  ...
                  <label className="flex items-center gap-2 mt-2 text-sm text-gray-400 cursor-pointer">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setFormData({
                          ...formData,
                          notify_user_by_email: e.target.checked,
                        })
                      }
                ...
                <label className="flex items-center gap-2 text-sm text-gray-400 cursor-pointer">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        notify_tenant_by_email: e.target.checked,
                      })
                    }
                    className="w-4 h-4 text-primary-blue rounded focus:ring-blue-500"
                  />{" "}
