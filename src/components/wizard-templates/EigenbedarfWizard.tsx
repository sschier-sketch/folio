...
import {
  generateEigenbedarfPdf,
  buildEigenbedarfFilename,
} from '../../lib/kuendigungEigenbedarfPdfGenerator';
import type {
  EigenbedarfStep,
  EigenbedarfWizardData,
  LandlordData,
  TenantEntry,
} from './types';
import { EIGENBEDARF_STEPS } from './types';
...
import StepEigenbedarfSachverhalt, {
  isEigenbedarfSachverhaltValid,
} from './StepEigenbedarfSachverhalt';
...
const TEMPLATE_ID = 'kuendigung_eigenbedarf';

function emptyWizardData(): EigenbedarfWizardData {
  return {
    landlord: {
      name: '',
      street: '',
    ...
    sachverhalt: {
      ...
      raeumungsgrund: '',
      hatAndereWohnung: '',
      andereWohnungNutzbar: '',
    },
  };
}

export default function EigenbedarfWizard({ onBack, freshStart }: Props) {
  const { user } = useAuth();
  const [step, setStep] = useState<EigenbedarfStep>('vermieter');
  const [data, setData] = useState<EigenbedarfWizardData>(emptyWizardData());
  ...
  async function initWizard() {
    ...
    let draftData: EigenbedarfWizardData | null = null;
    ...
    if (!freshStart) {
      ...
      if (draft) {
        const saved = draft.draft_data as EigenbedarfWizardData;
        if (saved.landlord) {
    ...
    if (draftData && draftRow) {
      ...
      const merged: EigenbedarfWizardData = {
        ...draftData,
        landlord: {
          ...draftData.landlord,
      ...
      if (si >= 0 && si < EIGENBEDARF_STEPS.length) {
        setStep(EIGENBEDARF_STEPS[si].key);
      }
    } else {
      setData((prev) => ({
        ...prev,
  ...
  const saveDraft = useCallback(
    async (currentData: EigenbedarfWizardData, currentStep: EigenbedarfStep) => {
      if (!user) return;
      const stepIdx = EIGENBEDARF_STEPS.findIndex((s) => s.key === currentStep);
      const payload = {
        user_id: user.id,
  ...
  const stepIdx = EIGENBEDARF_STEPS.findIndex((s) => s.key === step);
  ...
  function canProceed(): boolean {
    switch (step) {
      ...
      case 'ansprache':
        return true;
      case 'sachverhalt':
        return isEigenbedarfSachverhaltValid(data.sachverhalt);
      default:
        return true;
    }
  }
  ...
  async function goNext() {
    ...
    if (nextIdx < EIGENBEDARF_STEPS.length) {
      const nextStep = EIGENBEDARF_STEPS[nextIdx].key;
      setStep(nextStep);
  ...
  function goPrev() {
    ...
    if (prevIdx >= 0) {
      setStep(EIGENBEDARF_STEPS[prevIdx].key);
    } else {
      onBack();
    }
  }
  ...
  async function handleDownload() {
    setGenerating(true);
    try {
      const blob = generateEigenbedarfPdf(data);
      const filename = buildEigenbedarfFilename(data.tenants);
          ...
          if (tenantRow?.email) {
            ...
            await fetch(`${supabaseUrl}/functions/v1/send-email`, {
              ...
              body: JSON.stringify({
                ...
                text: nachricht,
                userId: user!.id,
                mailType: 'kuendigung_eigenbedarf',
  ...
  async function saveDocumentToProperty(blob: Blob, filename: string, shareWithTenant: boolean): Promise<string | null> {
    ...
    const { data: docRow } = await supabase
      ...
      .insert({
        ...
        document_type: 'kuendigung_eigenbedarf',
        file_size: blob.size,
        shared_with_tenant: shareWithTenant,
        description: 'Kündigung wegen Eigenbedarf (Assistent)',
      })
      .select('id')
      .maybeSingle();
  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <h1 className="text-3xl font-bold text-dark mb-2">Kündigung wegen Eigenbedarf erstellen</h1>
        <p className="text-gray-400">
          Erstellen Sie Schritt für Schritt eine Kündigung wegen Eigenbedarfs gemäß §§ 573 Abs. 1, 573 Abs. 2 Nr. 2 BGB.
        </p>
      </div>
