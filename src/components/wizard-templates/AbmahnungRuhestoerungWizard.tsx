...
import {
  generateAbmahnungRuhestoerungPdf,
  buildAbmahnungFilename,
} from '../../lib/abmahnungRuhestoerungPdfGenerator';
import type {
  AbmahnungRuhestoerungStep,
  AbmahnungRuhestoerungWizardData,
  LandlordData,
  TenantEntry,
} from './types';
import { ABMAHNUNG_RUHESTOERUNG_STEPS } from './types';
...
import StepAbmahnungSachverhalt, { isAbmahnungSachverhaltValid } from './StepAbmahnungSachverhalt';
...
function emptyWizardData(): AbmahnungRuhestoerungWizardData {
  return {
    landlord: {
      name: '',
      street: '',
...
const TEMPLATE_ID = 'abmahnung_ruhestoerung';

export default function AbmahnungRuhestoerungWizard({ onBack, freshStart }: Props) {
  const { user } = useAuth();
  const [step, setStep] = useState<AbmahnungRuhestoerungStep>('vermieter');
  const [data, setData] = useState<AbmahnungRuhestoerungWizardData>(emptyWizardData());
  ...
  async function initWizard() {
    ...
    let draftData: AbmahnungRuhestoerungWizardData | null = null;
    ...
    if (!freshStart) {
      ...
      if (draft) {
        const saved = draft.draft_data as AbmahnungRuhestoerungWizardData;
        if (saved.landlord) {
    ...
    if (draftData && draftRow) {
      ...
      const merged: AbmahnungRuhestoerungWizardData = {
        ...draftData,
        landlord: {
          ...draftData.landlord,
      ...
      if (si >= 0 && si < ABMAHNUNG_RUHESTOERUNG_STEPS.length) {
        setStep(ABMAHNUNG_RUHESTOERUNG_STEPS[si].key);
      }
    } else {
      setData((prev) => ({
        ...prev,
  ...
  const saveDraft = useCallback(
    async (currentData: AbmahnungRuhestoerungWizardData, currentStep: AbmahnungRuhestoerungStep) => {
      if (!user) return;
      const stepIdx = ABMAHNUNG_RUHESTOERUNG_STEPS.findIndex((s) => s.key === currentStep);
      const payload = {
        user_id: user.id,
  ...
  const stepIdx = ABMAHNUNG_RUHESTOERUNG_STEPS.findIndex((s) => s.key === step);
  ...
  function canProceed(): boolean {
    switch (step) {
      ...
      case 'ansprache':
        return true;
      case 'sachverhalt':
        return isAbmahnungSachverhaltValid(data.sachverhalt);
      default:
        return true;
    }
  }
  ...
  async function goNext() {
    ...
    if (nextIdx < ABMAHNUNG_RUHESTOERUNG_STEPS.length) {
      const nextStep = ABMAHNUNG_RUHESTOERUNG_STEPS[nextIdx].key;
      setStep(nextStep);
  ...
  function goPrev() {
    ...
    if (prevIdx >= 0) {
      setStep(ABMAHNUNG_RUHESTOERUNG_STEPS[prevIdx].key);
    } else {
      onBack();
    }
  }
  ...
  async function handleDownload() {
    setGenerating(true);
    try {
      const blob = generateAbmahnungRuhestoerungPdf(data);
      const filename = buildAbmahnungFilename(data.tenants);
          ...
          if (tenantRow?.email) {
            ...
            await fetch(`${supabaseUrl}/functions/v1/send-email`, {
              ...
              body: JSON.stringify({
                ...
                text: nachricht,
                userId: user!.id,
                mailType: 'abmahnung_ruhestoerung',
  ...
  async function saveDocumentToProperty(blob: Blob, filename: string, shareWithTenant: boolean): Promise<string | null> {
    ...
    const { data: docRow } = await supabase
      ...
      .insert({
        ...
        document_type: 'abmahnung_ruhestoerung',
        file_size: blob.size,
        shared_with_tenant: shareWithTenant,
        description: 'Abmahnung wegen Ruhestörung (Assistent)',
      })
      .select('id')
      .maybeSingle();
  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <h1 className="text-3xl font-bold text-dark mb-2">Abmahnung wegen Ruhestörung erstellen</h1>
        <p className="text-gray-400">
          Erstellen Sie Schritt für Schritt eine formelle Abmahnung bei Ruhestörung.
        </p>
      </div>
      ...
      <div className="flex gap-8">
        <div className="w-52 flex-shrink-0 hidden md:block">
          <nav className="space-y-1 sticky top-4">
            {ABMAHNUNG_RUHESTOERUNG_STEPS.map((s, i) => {
          ...
          <div className="bg-white rounded-lg p-6 lg:p-8 min-h-[400px]">
            ...
            {step === 'versand' && (
              <StepVersand
                ...
                portalEnabled={true}
                onSend={handleSend}
                defaultSubject="Abmahnung wegen Ruhestörung"
                defaultMessage={`Sehr geehrte/r ${tenantNames},\n\nanbei erhalten Sie eine Abmahnung wegen Ruhestörung für Ihr Mietverhältnis.\n\nMit freundlichen Grüßen`}
