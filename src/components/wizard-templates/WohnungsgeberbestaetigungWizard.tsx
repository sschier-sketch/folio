...
import {
  generateWohnungsgeberbestaetigungPdf,
  buildWohnungsgeberFilename,
} from '../../lib/wohnungsgeberbestaetigungPdfGenerator';
import type {
  WohnungsgeberStep,
  WohnungsgeberWizardData,
  LandlordData,
} from './types';
import { WOHNUNGSGEBER_STEPS } from './types';
import StepVermieter, { isVermieterValid } from './StepVermieter';
import StepMieter, { isMieterValid } from './StepMieter';
import StepWohnungsgeberSachverhalt, {
  isWohnungsgeberSachverhaltValid,
} from './StepWohnungsgeberSachverhalt';
...
const TEMPLATE_ID = 'wohnungsgeberbestaetigung';

function emptyWizardData(): WohnungsgeberWizardData {
  return {
    landlord: {
      name: '',
      street: '',
...
export default function WohnungsgeberbestaetigungWizard({ onBack, freshStart }: Props) {
  const { user } = useAuth();
  const [step, setStep] = useState<WohnungsgeberStep>('vermieter');
  const [data, setData] = useState<WohnungsgeberWizardData>(emptyWizardData());
  ...
  async function initWizard() {
    ...
    let draftData: WohnungsgeberWizardData | null = null;
    ...
    if (!freshStart) {
      ...
      if (draft) {
        const saved = draft.draft_data as WohnungsgeberWizardData;
        if (saved.landlord) {
    ...
    if (draftData && draftRow) {
      ...
      const merged: WohnungsgeberWizardData = {
        ...draftData,
        landlord: {
          ...draftData.landlord,
      ...
      if (si >= 0 && si < WOHNUNGSGEBER_STEPS.length) {
        setStep(WOHNUNGSGEBER_STEPS[si].key);
      }
    } else {
      setData((prev) => ({
        ...prev,
  ...
  const saveDraft = useCallback(
    async (currentData: WohnungsgeberWizardData, currentStep: WohnungsgeberStep) => {
      if (!user) return;
      const stepIdx = WOHNUNGSGEBER_STEPS.findIndex((s) => s.key === currentStep);
      const payload = {
        user_id: user.id,
  ...
  const stepIdx = WOHNUNGSGEBER_STEPS.findIndex((s) => s.key === step);
  ...
  function canProceed(): boolean {
    switch (step) {
      ...
      case 'sachverhalt':
        return isWohnungsgeberSachverhaltValid(data.sachverhalt);
      default:
        return true;
    }
  }
  ...
  async function goNext() {
    ...
    if (nextIdx < WOHNUNGSGEBER_STEPS.length) {
      const nextStep = WOHNUNGSGEBER_STEPS[nextIdx].key;
      setStep(nextStep);
  ...
  function goPrev() {
    ...
    if (prevIdx >= 0) {
      setStep(WOHNUNGSGEBER_STEPS[prevIdx].key);
    } else {
      onBack();
    }
  }
  ...
  async function handleDownload() {
    setGenerating(true);
    try {
      const blob = generateWohnungsgeberbestaetigungPdf(data);
      const filename = buildWohnungsgeberFilename(data.tenants);
          ...
          if (tenantRow?.email) {
            ...
            await fetch(`${supabaseUrl}/functions/v1/send-email`, {
              ...
              body: JSON.stringify({
                ...
                text: nachricht,
                userId: user!.id,
                mailType: 'wohnungsgeberbestaetigung',
  ...
  async function saveDocumentToProperty(
    ...
    const { data: docRow } = await supabase
      ...
      .insert({
        ...
        description: 'Wohnungsgeberbestätigung (Assistent)',
      })
      .select('id')
      .maybeSingle();
          ...
          <div className="bg-white rounded-lg p-6 lg:p-8 min-h-[400px]">
            ...
            {step === 'versand' && (
              <StepVersand
                ...
                portalEnabled={true}
                onSend={handleSend}
                defaultSubject="Wohnungsgeberbestätigung"
                documentLabel="Wohnungsgeberbestätigung"
              />
            )}
          </div>
