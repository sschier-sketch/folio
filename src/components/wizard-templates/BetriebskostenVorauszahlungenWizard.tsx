...
import {
  generateBetriebskostenPdf,
  buildBetriebskostenFilename,
} from '../../lib/betriebskostenPdfGenerator';
import type {
  BetriebskostenStep,
  BetriebskostenWizardData,
  LandlordData,
  TenantEntry,
} from './types';
import { BETRIEBSKOSTEN_STEPS } from './types';
...
import StepBetriebskostenSachverhalt, { isBetriebskostenSachverhaltValid } from './StepBetriebskostenSachverhalt';
...
function emptyWizardData(): BetriebskostenWizardData {
  return {
    landlord: {
      name: '',
      street: '',
...
const TEMPLATE_ID = 'betriebskosten_vorauszahlungen';

export default function BetriebskostenVorauszahlungenWizard({ onBack, freshStart }: Props) {
  const { user } = useAuth();
  const [step, setStep] = useState<BetriebskostenStep>('vermieter');
  const [data, setData] = useState<BetriebskostenWizardData>(emptyWizardData());
  ...
  async function initWizard() {
    ...
    let draftData: BetriebskostenWizardData | null = null;
    ...
    if (!freshStart) {
      ...
      if (draft) {
        const saved = draft.draft_data as BetriebskostenWizardData;
        if (saved.landlord) {
    ...
    if (draftData && draftRow) {
      ...
      const merged: BetriebskostenWizardData = {
        ...draftData,
        landlord: {
          ...draftData.landlord,
      ...
      if (si >= 0 && si < BETRIEBSKOSTEN_STEPS.length) {
        setStep(BETRIEBSKOSTEN_STEPS[si].key);
      }
    } else {
      setData((prev) => ({
        ...prev,
  ...
  const saveDraft = useCallback(
    async (currentData: BetriebskostenWizardData, currentStep: BetriebskostenStep) => {
      if (!user) return;
      const stepIdx = BETRIEBSKOSTEN_STEPS.findIndex((s) => s.key === currentStep);
      const payload = {
        user_id: user.id,
  ...
  const stepIdx = BETRIEBSKOSTEN_STEPS.findIndex((s) => s.key === step);
  ...
  function canProceed(): boolean {
    switch (step) {
      ...
      case 'ansprache':
        return true;
      case 'sachverhalt':
        return isBetriebskostenSachverhaltValid(data.sachverhalt);
      default:
        return true;
    }
  }
  ...
  async function goNext() {
    ...
    if (nextIdx < BETRIEBSKOSTEN_STEPS.length) {
      const nextStep = BETRIEBSKOSTEN_STEPS[nextIdx].key;
      setStep(nextStep);
  ...
  function goPrev() {
    ...
    if (prevIdx >= 0) {
      setStep(BETRIEBSKOSTEN_STEPS[prevIdx].key);
    } else {
      onBack();
    }
  }
  ...
  async function handleDownload() {
    setGenerating(true);
    try {
      const blob = generateBetriebskostenPdf(data);
      const filename = buildBetriebskostenFilename(data.tenants);
          ...
          if (tenantRow?.email) {
            ...
            await fetch(`${supabaseUrl}/functions/v1/send-email`, {
              ...
              body: JSON.stringify({
                ...
                text: nachricht,
                userId: user!.id,
                mailType: 'betriebskosten_vorauszahlungen',
  ...
  async function saveDocumentToProperty(blob: Blob, filename: string, shareWithTenant: boolean): Promise<string | null> {
    ...
    const { data: docRow } = await supabase
      ...
      .insert({
        ...
        document_type: 'betriebskosten_vorauszahlungen',
        file_size: blob.size,
        shared_with_tenant: shareWithTenant,
        description: 'Anpassung Betriebskostenvorauszahlungen (Assistent)',
      })
      .select('id')
      .maybeSingle();
  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <h1 className="text-3xl font-bold text-dark mb-2">Anpassung Betriebskostenvorauszahlungen erstellen</h1>
        <p className="text-gray-400">
          Erstellen Sie Schritt für Schritt ein Schreiben zur Anpassung der monatlichen Betriebskostenvorauszahlungen.
        </p>
      </div>
          ...
          <div className="bg-white rounded-lg p-6 lg:p-8 min-h-[400px]">
            ...
            {step === 'versand' && (
              <StepVersand
                ...
                portalEnabled={true}
                onSend={handleSend}
                defaultSubject="Anpassung Ihrer Betriebskostenvorauszahlungen"
                defaultMessage={`Sehr geehrte/r ${tenantNames},\n\nanbei erhalten Sie ein Schreiben zur Anpassung Ihrer monatlichen Betriebskostenvorauszahlungen.\n\nMit freundlichen Grüßen`}
                documentLabel="Anpassungsschreiben"
              />
            )}
          </div>
