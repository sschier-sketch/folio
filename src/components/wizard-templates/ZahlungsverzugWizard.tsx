...
import {
  generateZahlungsverzugPdf,
  buildZahlungsverzugFilename,
} from '../../lib/kuendigungZahlungsverzugPdfGenerator';
import type {
  ZahlungsverzugStep,
  ZahlungsverzugWizardData,
  LandlordData,
  TenantEntry,
} from './types';
import { ZAHLUNGSVERZUG_STEPS } from './types';
...
import StepZahlungsverzugSachverhalt, {
  isZahlungsverzugSachverhaltValid,
} from './StepZahlungsverzugSachverhalt';
...
const TEMPLATE_ID = 'kuendigung_zahlungsverzug';

function emptyWizardData(): ZahlungsverzugWizardData {
  return {
    landlord: {
      name: '',
      street: '',
    ...
    sachverhalt: {
      ...
      zahlungen: [{ betrag: '', faelligkeitsdatum: '' }],
      rueckgabeDatum: '',
    },
  };
}

export default function ZahlungsverzugWizard({ onBack, freshStart }: Props) {
  const { user } = useAuth();
  const [step, setStep] = useState<ZahlungsverzugStep>('vermieter');
  const [data, setData] = useState<ZahlungsverzugWizardData>(emptyWizardData());
  ...
  async function initWizard() {
    ...
    let draftData: ZahlungsverzugWizardData | null = null;
    ...
    if (!freshStart) {
      ...
      if (draft) {
        const saved = draft.draft_data as ZahlungsverzugWizardData;
        if (saved.landlord) {
    ...
    if (draftData && draftRow) {
      ...
      const merged: ZahlungsverzugWizardData = {
        ...draftData,
        landlord: {
          ...draftData.landlord,
      ...
      if (si >= 0 && si < ZAHLUNGSVERZUG_STEPS.length) {
        setStep(ZAHLUNGSVERZUG_STEPS[si].key);
      }
    } else {
      setData((prev) => ({
        ...prev,
  ...
  const saveDraft = useCallback(
    async (currentData: ZahlungsverzugWizardData, currentStep: ZahlungsverzugStep) => {
      if (!user) return;
      const stepIdx = ZAHLUNGSVERZUG_STEPS.findIndex((s) => s.key === currentStep);
      const payload = {
        user_id: user.id,
  ...
  const stepIdx = ZAHLUNGSVERZUG_STEPS.findIndex((s) => s.key === step);
  ...
  function canProceed(): boolean {
    switch (step) {
      ...
      case 'ansprache':
        return true;
      case 'sachverhalt':
        return isZahlungsverzugSachverhaltValid(data.sachverhalt);
      default:
        return true;
    }
  }
  ...
  async function goNext() {
    ...
    if (nextIdx < ZAHLUNGSVERZUG_STEPS.length) {
      const nextStep = ZAHLUNGSVERZUG_STEPS[nextIdx].key;
      setStep(nextStep);
  ...
  function goPrev() {
    ...
    if (prevIdx >= 0) {
      setStep(ZAHLUNGSVERZUG_STEPS[prevIdx].key);
    } else {
      onBack();
    }
  }
  ...
  async function handleDownload() {
    setGenerating(true);
    try {
      const blob = generateZahlungsverzugPdf(data);
      const filename = buildZahlungsverzugFilename(data.tenants);
          ...
          if (tenantRow?.email) {
            ...
            await fetch(`${supabaseUrl}/functions/v1/send-email`, {
              ...
              body: JSON.stringify({
                ...
                text: nachricht,
                userId: user!.id,
                mailType: 'kuendigung_zahlungsverzug',
  ...
  async function saveDocumentToProperty(blob: Blob, filename: string, shareWithTenant: boolean): Promise<string | null> {
    ...
    const { data: docRow } = await supabase
      ...
      .insert({
        ...
        document_type: 'kuendigung_zahlungsverzug',
        file_size: blob.size,
        shared_with_tenant: shareWithTenant,
        description: 'Kündigung wegen Zahlungsverzug (Assistent)',
      })
      .select('id')
      .maybeSingle();
  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <h1 className="text-3xl font-bold text-dark mb-2">Kündigung wegen Zahlungsverzug erstellen</h1>
        <p className="text-gray-400">
          Erstellen Sie Schritt für Schritt eine fristlose Kündigung wegen Zahlungsverzug gemäß § 543 BGB.
        </p>
      </div>
