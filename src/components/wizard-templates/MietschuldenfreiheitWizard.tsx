...
import {
  generateMietschuldenfreiheitPdf,
  buildMietschuldenfreiheitFilename,
} from '../../lib/mietschuldenfreiheitPdfGenerator';
import type {
  MietschuldenfreiheitStep,
  MietschuldenfreiheitWizardData,
  LandlordData,
} from './types';
import { MIETSCHULDENFREIHEIT_STEPS } from './types';
import StepVermieter, { isVermieterValid } from './StepVermieter';
import StepMieter, { isMieterValid } from './StepMieter';
import StepMietschuldenfreiheitSachverhalt, {
  isMietschuldenfreiheitSachverhaltValid,
} from './StepMietschuldenfreiheitSachverhalt';
...
const TEMPLATE_ID = 'mietschuldenfreiheit';

function emptyWizardData(): MietschuldenfreiheitWizardData {
  return {
    landlord: {
      name: '',
      street: '',
...
export default function MietschuldenfreiheitWizard({ onBack, freshStart }: Props) {
  const { user } = useAuth();
  const [step, setStep] = useState<MietschuldenfreiheitStep>('vermieter');
  const [data, setData] = useState<MietschuldenfreiheitWizardData>(emptyWizardData());
  ...
  async function initWizard() {
    ...
    let draftData: MietschuldenfreiheitWizardData | null = null;
    ...
    if (!freshStart) {
      ...
      if (draft) {
        const saved = draft.draft_data as MietschuldenfreiheitWizardData;
        if (saved.landlord) {
    ...
    if (draftData && draftRow) {
      ...
      const merged: MietschuldenfreiheitWizardData = {
        ...draftData,
        landlord: {
          ...draftData.landlord,
      ...
      if (si >= 0 && si < MIETSCHULDENFREIHEIT_STEPS.length) {
        setStep(MIETSCHULDENFREIHEIT_STEPS[si].key);
      }
    } else {
      setData((prev) => ({
        ...prev,
  ...
  const saveDraft = useCallback(
    async (currentData: MietschuldenfreiheitWizardData, currentStep: MietschuldenfreiheitStep) => {
      if (!user) return;
      const stepIdx = MIETSCHULDENFREIHEIT_STEPS.findIndex((s) => s.key === currentStep);
      const payload = {
        user_id: user.id,
  ...
  const stepIdx = MIETSCHULDENFREIHEIT_STEPS.findIndex((s) => s.key === step);
  ...
  function canProceed(): boolean {
    switch (step) {
      ...
      case 'sachverhalt':
        return isMietschuldenfreiheitSachverhaltValid(data.sachverhalt);
      default:
        return true;
    }
  }
  ...
  async function goNext() {
    ...
    if (nextIdx < MIETSCHULDENFREIHEIT_STEPS.length) {
      const nextStep = MIETSCHULDENFREIHEIT_STEPS[nextIdx].key;
      setStep(nextStep);
  ...
  function goPrev() {
    ...
    if (prevIdx >= 0) {
      setStep(MIETSCHULDENFREIHEIT_STEPS[prevIdx].key);
    } else {
      onBack();
    }
  }
  ...
  async function handleDownload() {
    setGenerating(true);
    try {
      const blob = generateMietschuldenfreiheitPdf(data);
      const filename = buildMietschuldenfreiheitFilename(data.tenants);
  ...
  async function saveDocumentToProperty(
    blob: Blob,
    filename: string,
    ...
    const { data: docRow } = await supabase
      ...
      .insert({
        ...
        document_type: 'mietschuldenfreiheit',
        file_size: blob.size,
        shared_with_tenant: shareWithTenant,
        description: 'Mietschuldenfreiheitsbescheinigung (Assistent)',
      })
      .select('id')
      .maybeSingle();

    if (docRow?.id && primaryTenant?.propertyId) {
      const associations = [];
      associations.push({
    ...
    <div>
      <div className="mb-8">
        ...
        <h1 className="text-3xl font-bold text-dark mb-2">
          Mietschuldenfreiheitsbescheinigung erstellen
        </h1>
        <p className="text-gray-400">
          Erstellen Sie eine Bescheinigung über den Schuldenstand Ihres Mieters.
        </p>
      </div>
      ...
      <div className="flex gap-8">
        <div className="w-52 flex-shrink-0 hidden md:block">
          <nav className="space-y-1 sticky top-4">
            {MIETSCHULDENFREIHEIT_STEPS.map((s, i) => {
        ...
        <div className="flex-1 min-w-0">
          <div className="bg-white rounded-lg p-6 lg:p-8 min-h-[400px]">
            ...
            {step === 'mieter' && (
              <StepMieter
                ...
                onChange={(tenants) => setData({ ...data, tenants })}
              />
            )}

            {step === 'sachverhalt' && (
              <StepMietschuldenfreiheitSachverhalt
            ...
            {step === 'versand' && (
              <StepVersand
                ...
                portalEnabled={true}
                onSend={handleSend}
                defaultSubject="Mietschuldenfreiheitsbescheinigung"
                documentLabel="Mietschuldenfreiheitsbescheinigung"
              />
            )}
          </div>
          ...
          {step === 'ergebnis' && (
            <div className="flex items-center justify-between mt-6">
              <Button onClick={() => setStep('sachverhalt')} variant="outlined">
                Zurück
              </Button>
            </div>
          )}
