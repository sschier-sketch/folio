...
import {
  generateKuendigungAbmahnungPdf,
  buildKuendigungAbmahnungFilename,
} from '../../lib/kuendigungAbmahnungPdfGenerator';
import type {
  KuendigungAbmahnungStep,
  KuendigungAbmahnungWizardData,
  LandlordData,
  TenantEntry,
} from './types';
import { KUENDIGUNG_ABMAHNUNG_STEPS } from './types';
...
import StepKuendigungAbmahnungSachverhalt, {
  isKuendigungAbmahnungSachverhaltValid,
} from './StepKuendigungAbmahnungSachverhalt';
...
const TEMPLATE_ID = 'kuendigung_abmahnung';

function emptyWizardData(): KuendigungAbmahnungWizardData {
  return {
    landlord: {
      name: '',
      street: '',
...
export default function KuendigungAbmahnungWizard({ onBack, freshStart }: Props) {
  const { user } = useAuth();
  const [step, setStep] = useState<KuendigungAbmahnungStep>('vermieter');
  const [data, setData] = useState<KuendigungAbmahnungWizardData>(emptyWizardData());
  ...
  async function initWizard() {
    ...
    let draftData: KuendigungAbmahnungWizardData | null = null;
    ...
    if (!freshStart) {
      ...
      if (draft) {
        const saved = draft.draft_data as KuendigungAbmahnungWizardData;
        if (saved.landlord) {
    ...
    if (draftData && draftRow) {
      ...
      const merged: KuendigungAbmahnungWizardData = {
        ...draftData,
        landlord: {
          ...draftData.landlord,
      ...
      if (si >= 0 && si < KUENDIGUNG_ABMAHNUNG_STEPS.length) {
        setStep(KUENDIGUNG_ABMAHNUNG_STEPS[si].key);
      }
    } else {
      setData((prev) => ({
        ...prev,
  ...
  const saveDraft = useCallback(
    async (currentData: KuendigungAbmahnungWizardData, currentStep: KuendigungAbmahnungStep) => {
      if (!user) return;
      const stepIdx = KUENDIGUNG_ABMAHNUNG_STEPS.findIndex((s) => s.key === currentStep);
      const payload = {
        user_id: user.id,
  ...
  const stepIdx = KUENDIGUNG_ABMAHNUNG_STEPS.findIndex((s) => s.key === step);
  ...
  function canProceed(): boolean {
    switch (step) {
      ...
      case 'ansprache':
        return true;
      case 'sachverhalt':
        return isKuendigungAbmahnungSachverhaltValid(data.sachverhalt);
      default:
        return true;
    }
  }
  ...
  async function goNext() {
    ...
    if (nextIdx < KUENDIGUNG_ABMAHNUNG_STEPS.length) {
      const nextStep = KUENDIGUNG_ABMAHNUNG_STEPS[nextIdx].key;
      setStep(nextStep);
  ...
  function goPrev() {
    ...
    if (prevIdx >= 0) {
      setStep(KUENDIGUNG_ABMAHNUNG_STEPS[prevIdx].key);
    } else {
      onBack();
    }
  }
  ...
  async function handleDownload() {
    setGenerating(true);
    try {
      const blob = generateKuendigungAbmahnungPdf(data);
      const filename = buildKuendigungAbmahnungFilename(data.tenants);
  ...
  async function saveDocumentToProperty(blob: Blob, filename: string, shareWithTenant: boolean): Promise<string | null> {
    ...
    const { data: docRow } = await supabase
      ...
      .insert({
        ...
        document_type: 'kuendigung_abmahnung',
        file_size: blob.size,
        shared_with_tenant: shareWithTenant,
        description: 'Kündigung nach Abmahnung (Assistent)',
      })
      .select('id')
      .maybeSingle();
  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <h1 className="text-3xl font-bold text-dark mb-2">Kündigung nach Abmahnung erstellen</h1>
        <p className="text-gray-400">
          Erstellen Sie Schritt für Schritt eine fristlose Kündigung nach erfolgloser Abmahnung gemäß § 543 BGB.
        </p>
      </div>
      ...
      <div className="flex gap-8">
        <div className="w-52 flex-shrink-0 hidden md:block">
          <nav className="space-y-1 sticky top-4">
            {KUENDIGUNG_ABMAHNUNG_STEPS.map((s, i) => {
        ...
        <div className="flex-1 min-w-0">
          <div className="bg-white rounded-lg p-6 lg:p-8 min-h-[400px]">
            ...
            {step === 'sachverhalt' && (
              <StepKuendigungAbmahnungSachverhalt
