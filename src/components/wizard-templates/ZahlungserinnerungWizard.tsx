...
import {
  generateZahlungserinnerungPdf,
  buildZahlungserinnerungFilename,
} from '../../lib/zahlungserinnerungPdfGenerator';
import type {
  ZahlungserinnerungStep,
  ZahlungserinnerungWizardData,
  LandlordData,
  TenantEntry,
} from './types';
import { ZAHLUNGSERINNERUNG_STEPS } from './types';
...
import StepZahlungSachverhalt, { isZahlungSachverhaltValid } from './StepZahlungSachverhalt';
...
function emptyWizardData(): ZahlungserinnerungWizardData {
  return {
    landlord: {
      name: '',
      street: '',
...
const TEMPLATE_ID = 'zahlungserinnerung';

export default function ZahlungserinnerungWizard({ onBack, freshStart }: Props) {
  const { user } = useAuth();
  const [step, setStep] = useState<ZahlungserinnerungStep>('vermieter');
  const [data, setData] = useState<ZahlungserinnerungWizardData>(emptyWizardData());
  ...
  async function initWizard() {
    ...
    let draftData: ZahlungserinnerungWizardData | null = null;
    ...
    if (!freshStart) {
      ...
      if (draft) {
        const saved = draft.draft_data as ZahlungserinnerungWizardData;
        if (saved.landlord) {
    ...
    if (draftData && draftRow) {
      ...
      const merged: ZahlungserinnerungWizardData = {
        ...draftData,
        landlord: {
          ...draftData.landlord,
      ...
      if (si >= 0 && si < ZAHLUNGSERINNERUNG_STEPS.length) {
        setStep(ZAHLUNGSERINNERUNG_STEPS[si].key);
      }
    } else {
      setData((prev) => ({
        ...prev,
  ...
  const saveDraft = useCallback(
    async (currentData: ZahlungserinnerungWizardData, currentStep: ZahlungserinnerungStep) => {
      if (!user) return;
      const stepIdx = ZAHLUNGSERINNERUNG_STEPS.findIndex((s) => s.key === currentStep);
      const payload = {
        user_id: user.id,
  ...
  const stepIdx = ZAHLUNGSERINNERUNG_STEPS.findIndex((s) => s.key === step);
  ...
  function canProceed(): boolean {
    switch (step) {
      ...
      case 'ansprache':
        return true;
      case 'sachverhalt':
        return isZahlungSachverhaltValid(data.sachverhalt);
      default:
        return true;
    }
  }
  ...
  async function goNext() {
    ...
    if (nextIdx < ZAHLUNGSERINNERUNG_STEPS.length) {
      const nextStep = ZAHLUNGSERINNERUNG_STEPS[nextIdx].key;
      setStep(nextStep);
  ...
  function goPrev() {
    ...
    if (prevIdx >= 0) {
      setStep(ZAHLUNGSERINNERUNG_STEPS[prevIdx].key);
    } else {
      onBack();
    }
  }
  ...
  async function handleDownload() {
    setGenerating(true);
    try {
      const blob = generateZahlungserinnerungPdf(data);
      const filename = buildZahlungserinnerungFilename(data.tenants);
  ...
  async function saveDocumentToProperty(blob: Blob, filename: string, shareWithTenant: boolean): Promise<string | null> {
    ...
    const { data: docRow } = await supabase
      ...
      .insert({
        ...
        description: 'Zahlungserinnerung (Assistent)',
      })
      .select('id')
      .maybeSingle();
  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <h1 className="text-3xl font-bold text-dark mb-2">Zahlungserinnerung erstellen</h1>
        <p className="text-gray-400">
          Erstellen Sie Schritt für Schritt eine höfliche Zahlungserinnerung für ausstehende Mieten.
        </p>
      </div>
      ...
      <div className="flex gap-8">
        <div className="w-52 flex-shrink-0 hidden md:block">
          <nav className="space-y-1 sticky top-4">
            {ZAHLUNGSERINNERUNG_STEPS.map((s, i) => {
          ...
          <div className="bg-white rounded-lg p-6 lg:p-8 min-h-[400px]">
            ...
            {step === 'versand' && (
              <StepVersand
                ...
                portalEnabled={true}
                onSend={handleSend}
                defaultSubject="Erinnerung zur Zahlung ausstehender Mieten"
                defaultMessage={`Sehr geehrte/r ${tenantNames},\n\nanbei erhalten Sie eine Erinnerung zur Zahlung der ausstehenden Miete.\n\nMit freundlichen Grüßen`}
                documentLabel="Zahlungserinnerung"
              />
            )}
          </div>
