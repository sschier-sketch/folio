...
import {
  generateMietkautionPdf,
  buildMietkautionFilename,
} from '../../lib/mietkautionRueckgriffPdfGenerator';
import type {
  MietkautionStep,
  MietkautionWizardData,
  LandlordData,
  TenantEntry,
} from './types';
import { MIETKAUTION_STEPS } from './types';
...
import StepMietkautionSachverhalt, {
  isMietkautionSachverhaltValid,
} from './StepMietkautionSachverhalt';
...
const TEMPLATE_ID = 'mietkaution_rueckgriff';

function emptyWizardData(): MietkautionWizardData {
  return {
    landlord: {
      name: '',
      street: '',
...
export default function MietkautionWizard({ onBack, freshStart }: Props) {
  const { user } = useAuth();
  const [step, setStep] = useState<MietkautionStep>('vermieter');
  const [data, setData] = useState<MietkautionWizardData>(emptyWizardData());
  ...
  async function initWizard() {
    ...
    let draftData: MietkautionWizardData | null = null;
    ...
    if (!freshStart) {
      ...
      if (draft) {
        const saved = draft.draft_data as MietkautionWizardData;
        if (saved.landlord) {
    ...
    if (draftData && draftRow) {
      ...
      const merged: MietkautionWizardData = {
        ...draftData,
        landlord: {
          ...draftData.landlord,
      ...
      if (si >= 0 && si < MIETKAUTION_STEPS.length) {
        setStep(MIETKAUTION_STEPS[si].key);
      }
    } else {
      setData((prev) => ({
        ...prev,
  ...
  const saveDraft = useCallback(
    async (currentData: MietkautionWizardData, currentStep: MietkautionStep) => {
      if (!user) return;
      const stepIdx = MIETKAUTION_STEPS.findIndex((s) => s.key === currentStep);
      const payload = {
        user_id: user.id,
  ...
  const stepIdx = MIETKAUTION_STEPS.findIndex((s) => s.key === step);
  ...
  function canProceed(): boolean {
    switch (step) {
      ...
      case 'ansprache':
        return true;
      case 'sachverhalt':
        return isMietkautionSachverhaltValid(data.sachverhalt);
      default:
        return true;
    }
  }
  ...
  async function goNext() {
    ...
    if (nextIdx < MIETKAUTION_STEPS.length) {
      const nextStep = MIETKAUTION_STEPS[nextIdx].key;
      setStep(nextStep);
  ...
  function goPrev() {
    ...
    if (prevIdx >= 0) {
      setStep(MIETKAUTION_STEPS[prevIdx].key);
    } else {
      onBack();
    }
  }
  ...
  async function handleDownload() {
    setGenerating(true);
    try {
      const blob = generateMietkautionPdf(data);
      const filename = buildMietkautionFilename(data.tenants);
          ...
          if (tenantRow?.email) {
            ...
            await fetch(`${supabaseUrl}/functions/v1/send-email`, {
              ...
              body: JSON.stringify({
                ...
                text: nachricht,
                userId: user!.id,
                mailType: 'mietkaution_rueckgriff',
  ...
  async function saveDocumentToProperty(blob: Blob, filename: string, shareWithTenant: boolean): Promise<string | null> {
    ...
    const { data: docRow } = await supabase
      ...
      .insert({
        ...
        document_type: 'mietkaution_rueckgriff',
        file_size: blob.size,
        shared_with_tenant: shareWithTenant,
        description: 'Inanspruchnahme der Mietkaution (Assistent)',
      })
      .select('id')
      .maybeSingle();
    ...
    <div>
      <div className="mb-8">
        ...
        <h1 className="text-3xl font-bold text-dark mb-2">
          Rückgriff auf Mietkaution erstellen
        </h1>
        <p className="text-gray-400">
          Erstellen Sie Schritt für Schritt eine Abrechnung der Mietkaution mit Aufrechnung gegen offene Forderungen.
        </p>
      </div>
