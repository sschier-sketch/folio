...
import {
  generateAbmahnungBaulichePdf,
  buildAbmahnungBaulicheFilename,
} from '../../lib/abmahnungBaulichePdfGenerator';
import type {
  AbmahnungBaulicheStep,
  AbmahnungBaulicheWizardData,
  LandlordData,
  TenantEntry,
} from './types';
import { ABMAHNUNG_BAULICHE_STEPS } from './types';
...
import StepBaulicheSachverhalt, { isBaulicheSachverhaltValid } from './StepBaulicheSachverhalt';
...
function emptyWizardData(): AbmahnungBaulicheWizardData {
  return {
    landlord: {
      name: '',
      street: '',
...
const TEMPLATE_ID = 'abmahnung_bauliche_veraenderungen';

export default function AbmahnungBaulicheWizard({ onBack, freshStart }: Props) {
  const { user } = useAuth();
  const [step, setStep] = useState<AbmahnungBaulicheStep>('vermieter');
  const [data, setData] = useState<AbmahnungBaulicheWizardData>(emptyWizardData());
  ...
  async function initWizard() {
    ...
    let draftData: AbmahnungBaulicheWizardData | null = null;
    ...
    if (!freshStart) {
      ...
      if (draft) {
        const saved = draft.draft_data as AbmahnungBaulicheWizardData;
        if (saved.landlord) {
    ...
    if (draftData && draftRow) {
      ...
      const merged: AbmahnungBaulicheWizardData = {
        ...draftData,
        landlord: {
          ...draftData.landlord,
      ...
      if (si >= 0 && si < ABMAHNUNG_BAULICHE_STEPS.length) {
        setStep(ABMAHNUNG_BAULICHE_STEPS[si].key);
      }
    } else {
      setData((prev) => ({
        ...prev,
  ...
  const saveDraft = useCallback(
    async (currentData: AbmahnungBaulicheWizardData, currentStep: AbmahnungBaulicheStep) => {
      if (!user) return;
      const stepIdx = ABMAHNUNG_BAULICHE_STEPS.findIndex((s) => s.key === currentStep);
      const payload = {
        user_id: user.id,
  ...
  const stepIdx = ABMAHNUNG_BAULICHE_STEPS.findIndex((s) => s.key === step);
  ...
  function canProceed(): boolean {
    switch (step) {
      ...
      case 'ansprache':
        return true;
      case 'sachverhalt':
        return isBaulicheSachverhaltValid(data.sachverhalt);
      default:
        return true;
    }
  }
  ...
  async function goNext() {
    ...
    if (nextIdx < ABMAHNUNG_BAULICHE_STEPS.length) {
      const nextStep = ABMAHNUNG_BAULICHE_STEPS[nextIdx].key;
      setStep(nextStep);
  ...
  function goPrev() {
    ...
    if (prevIdx >= 0) {
      setStep(ABMAHNUNG_BAULICHE_STEPS[prevIdx].key);
    } else {
      onBack();
    }
  }
  ...
  async function handleDownload() {
    setGenerating(true);
    try {
      const blob = generateAbmahnungBaulichePdf(data);
      const filename = buildAbmahnungBaulicheFilename(data.tenants);
  ...
  async function handleSend(betreff: string, nachricht: string, sharePortal: boolean) {
    ...
    try {
      ...
      const attachments = !uploadErr ? [{ filename, path: emailAttachmentPath }] : [];
          ...
          if (tenantRow?.email) {
            ...
            await fetch(`${supabaseUrl}/functions/v1/send-email`, {
              ...
              body: JSON.stringify({
                ...
                text: nachricht,
                userId: user!.id,
                mailType: 'abmahnung_bauliche_veraenderungen',
  ...
  async function saveDocumentToProperty(blob: Blob, filename: string, shareWithTenant: boolean): Promise<string | null> {
    ...
    const { data: docRow } = await supabase
      ...
      .insert({
        ...
        document_type: 'abmahnung_bauliche_veraenderungen',
        file_size: blob.size,
        shared_with_tenant: shareWithTenant,
        description: 'Abmahnung wegen baulichen Veränderungen (Assistent)',
      })
      .select('id')
      .maybeSingle();
  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <h1 className="text-3xl font-bold text-dark mb-2">Abmahnung wegen baulichen Veränderungen erstellen</h1>
        <p className="text-gray-400">
          Erstellen Sie Schritt für Schritt eine formelle Abmahnung bei unerlaubten baulichen Veränderungen.
        </p>
      </div>
      ...
      <div className="flex gap-8">
        <div className="w-52 flex-shrink-0 hidden md:block">
          <nav className="space-y-1 sticky top-4">
            {ABMAHNUNG_BAULICHE_STEPS.map((s, i) => {
          ...
          <div className="bg-white rounded-lg p-6 lg:p-8 min-h-[400px]">
            ...
            {step === 'versand' && (
              <StepVersand
                ...
                portalEnabled={true}
                onSend={handleSend}
                defaultSubject="Abmahnung wegen unerlaubten baulichen Veränderungen"
                defaultMessage={`Sehr geehrte/r ${tenantNames},\n\nanbei erhalten Sie eine Abmahnung wegen unerlaubter baulicher Veränderungen an Ihrer Mietwohnung.\n\nMit freundlichen Grüßen`}
                documentLabel="Abmahnung"
              />
            )}
          </div>
