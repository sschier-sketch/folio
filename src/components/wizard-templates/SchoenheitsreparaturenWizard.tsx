...
import {
  generateSchoenheitsreparaturenPdf,
  buildSchoenheitsreparaturenFilename,
} from '../../lib/schoenheitsreparaturenPdfGenerator';
import type {
  SchoenheitsreparaturenStep,
  SchoenheitsreparaturenWizardData,
  LandlordData,
  TenantEntry,
} from './types';
import { SCHOENHEITSREPARATUREN_STEPS } from './types';
...
import StepSchoenheitsreparaturenSachverhalt, {
  isSchoenheitsreparaturenSachverhaltValid,
} from './StepSchoenheitsreparaturenSachverhalt';
...
const TEMPLATE_ID = 'schoenheitsreparaturen';

function emptyWizardData(): SchoenheitsreparaturenWizardData {
  return {
    landlord: {
      name: '',
      street: '',
...
export default function SchoenheitsreparaturenWizard({ onBack, freshStart }: Props) {
  const { user } = useAuth();
  const [step, setStep] = useState<SchoenheitsreparaturenStep>('vermieter');
  const [data, setData] = useState<SchoenheitsreparaturenWizardData>(emptyWizardData());
  ...
  async function initWizard() {
    ...
    let draftData: SchoenheitsreparaturenWizardData | null = null;
    ...
    if (!freshStart) {
      ...
      if (draft) {
        const saved = draft.draft_data as SchoenheitsreparaturenWizardData;
        if (saved.landlord) {
    ...
    if (draftData && draftRow) {
      ...
      const merged: SchoenheitsreparaturenWizardData = {
        ...draftData,
        landlord: {
          ...draftData.landlord,
      ...
      if (si >= 0 && si < SCHOENHEITSREPARATUREN_STEPS.length) {
        setStep(SCHOENHEITSREPARATUREN_STEPS[si].key);
      }
    } else {
      setData((prev) => ({
        ...prev,
  ...
  const saveDraft = useCallback(
    async (currentData: SchoenheitsreparaturenWizardData, currentStep: SchoenheitsreparaturenStep) => {
      if (!user) return;
      const stepIdx = SCHOENHEITSREPARATUREN_STEPS.findIndex((s) => s.key === currentStep);
      const payload = {
        user_id: user.id,
  ...
  const stepIdx = SCHOENHEITSREPARATUREN_STEPS.findIndex((s) => s.key === step);
  ...
  function canProceed(): boolean {
    switch (step) {
      ...
      case 'ansprache':
        return true;
      case 'sachverhalt':
        return isSchoenheitsreparaturenSachverhaltValid(data.sachverhalt);
      default:
        return true;
    }
  }
  ...
  async function goNext() {
    ...
    if (nextIdx < SCHOENHEITSREPARATUREN_STEPS.length) {
      const nextStep = SCHOENHEITSREPARATUREN_STEPS[nextIdx].key;
      setStep(nextStep);
  ...
  function goPrev() {
    ...
    if (prevIdx >= 0) {
      setStep(SCHOENHEITSREPARATUREN_STEPS[prevIdx].key);
    } else {
      onBack();
    }
  }
  ...
  async function handleDownload() {
    setGenerating(true);
    try {
      const blob = generateSchoenheitsreparaturenPdf(data);
      const filename = buildSchoenheitsreparaturenFilename(data.tenants);
          ...
          if (tenantRow?.email) {
            ...
            await fetch(`${supabaseUrl}/functions/v1/send-email`, {
              ...
              body: JSON.stringify({
                ...
                text: nachricht,
                userId: user!.id,
                mailType: 'schoenheitsreparaturen',
  ...
  async function saveDocumentToProperty(blob: Blob, filename: string, shareWithTenant: boolean): Promise<string | null> {
    ...
    const { data: docRow } = await supabase
      ...
      .insert({
        ...
        document_type: 'schoenheitsreparaturen',
        file_size: blob.size,
        shared_with_tenant: shareWithTenant,
        description: 'Aufforderung zu Schönheitsreparaturen (Assistent)',
      })
      .select('id')
      .maybeSingle();
    ...
    <div>
      <div className="mb-8">
        ...
        <h1 className="text-3xl font-bold text-dark mb-2">
          Aufforderung zu Schönheitsreparaturen erstellen
        </h1>
        <p className="text-gray-400">
          Erstellen Sie Schritt für Schritt eine Aufforderung zur Durchführung von Schönheitsreparaturen.
        </p>
      </div>
      ...
      <div className="flex gap-8">
        <div className="w-52 flex-shrink-0 hidden md:block">
          <nav className="space-y-1 sticky top-4">
            {SCHOENHEITSREPARATUREN_STEPS.map((s, i) => {
        ...
        <div className="flex-1 min-w-0">
          <div className="bg-white rounded-lg p-6 lg:p-8 min-h-[400px]">
            ...
            {step === 'sachverhalt' && (
              <StepSchoenheitsreparaturenSachverhalt
