...
import {
  generateMieterhoehungsverlangPdf,
  buildMieterhoehungsverlangenFilename,
} from '../../lib/mieterhoehungsverlangPdfGenerator';
import type {
  MieterhoehungStep,
  MieterhoehungWizardData,
  LandlordData,
  TenantEntry,
} from './types';
...
const TEMPLATE_ID = 'mieterhoehungsverlangen';

function emptyWizardData(): MieterhoehungWizardData {
  return {
    landlord: {
      name: '',
      street: '',
    ...
    sachverhalt: {
      ...
      begruendungMietspiegel: false,
      mietspiegelQualitaet: '',
      mietspiegelNeuerBetragProQm: '',
      begruendungVergleichswohnungen: false,
      vergleichswohnungen: [],
      begruendungGutachten: false,
      gutachterAnrede: '',
      gutachterName: '',
      gutachterBerichtsdatum: '',
      ersteMieterhoehung: null,
      letzteMieterhoehungDatum: '',
      modernisierung: null,
      modernisierungDatum: '',
      ruecksendeFrist: '',
    },
...
export default function MieterhoehungsverlangenWizard({ onBack, freshStart }: Props) {
  const { user } = useAuth();
  const [step, setStep] = useState<MieterhoehungStep>('vermieter');
  const [data, setData] = useState<MieterhoehungWizardData>(emptyWizardData());
  ...
  async function initWizard() {
    ...
    let draftData: MieterhoehungWizardData | null = null;
    ...
    if (!freshStart) {
      ...
      if (draft) {
        const saved = draft.draft_data as MieterhoehungWizardData;
        if (saved.landlord) {
    ...
    if (draftData && draftRow) {
      ...
      const merged: MieterhoehungWizardData = {
        ...draftData,
        landlord: {
          ...draftData.landlord,
      ...
      if (si >= 0 && si < MIETERHOEHUNG_STEPS.length) {
        setStep(MIETERHOEHUNG_STEPS[si].key);
      }
    } else {
      setData((prev) => ({
        ...prev,
  ...
  const saveDraft = useCallback(
    async (currentData: MieterhoehungWizardData, currentStep: MieterhoehungStep) => {
      if (!user) return;
      const stepIdx = MIETERHOEHUNG_STEPS.findIndex((s) => s.key === currentStep);
      const payload = {
        user_id: user.id,
  ...
  const stepIdx = MIETERHOEHUNG_STEPS.findIndex((s) => s.key === step);
  ...
  async function goNext() {
    ...
    if (nextIdx < MIETERHOEHUNG_STEPS.length) {
      const nextStep = MIETERHOEHUNG_STEPS[nextIdx].key;
      setStep(nextStep);
  ...
  function goPrev() {
    ...
    if (prevIdx >= 0) {
      setStep(MIETERHOEHUNG_STEPS[prevIdx].key);
    } else {
      onBack();
    }
  }
  ...
  async function handleDownload() {
    setGenerating(true);
    try {
      const blob = generateMieterhoehungsverlangPdf(data);
      const filename = buildMieterhoehungsverlangenFilename(data.tenants);
          ...
          if (tenantRow?.email) {
            ...
            await fetch(`${supabaseUrl}/functions/v1/send-email`, {
              ...
              body: JSON.stringify({
                ...
                text: nachricht,
                userId: user!.id,
                mailType: 'mieterhoehungsverlangen',
  ...
  async function saveDocumentToProperty(
    ...
    const { data: docRow } = await supabase
      ...
      .insert({
        ...
        document_type: 'mieterhoehungsverlangen',
        file_size: blob.size,
        shared_with_tenant: shareWithTenant,
        description: 'Mieterhöhungsverlangen (Assistent)',
      })
      .select('id')
      .maybeSingle();
    ...
    <div>
      <div className="mb-8">
        ...
        <p className="text-gray-400">
          Erstellen Sie Schritt für Schritt ein rechtssicheres Mieterhöhungsverlangen nach § 558 BGB.
        </p>
      </div>
