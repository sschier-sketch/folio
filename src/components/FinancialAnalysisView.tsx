...
import { TrendingUp, TrendingDown, DollarSign, Calendar, Calculator, Building2, CreditCard, BarChart3, PieChart } from 'lucide-react';
...
export default function FinancialAnalysisView() {
  ...
  const loadFinancialData = async () => {
    ...
    try {
      const [contractsRes, paymentsRes, loansRes, propertiesRes] = await Promise.all([
        supabase
        ...
        supabase
          .from('loans')
          .select('id, property_id, lender_name, monthly_payment, remaining_balance, interest_rate')
          .eq('user_id', user.id),
        supabase
          .from('properties')
      ...
      const propertyFinancialsData: PropertyFinancials[] = properties.map(property => {
        const propertyContracts = contracts.filter(c => c.property_id === property.id);
        const monthlyRent = propertyContracts.reduce(
          (sum, c) => sum + Number(c.base_rent) + Number(c.additional_costs || 0),
          0
        );
      ...
      const monthlyRent = propertyFinancialsData.reduce((sum, p) => sum + p.monthlyRent, 0);
      const totalRevenue = paymentsRes.data?.reduce((sum, p) => sum + Number(p.amount), 0) || 0;
      const loanPayments = loanFinancialsData.reduce((sum, l) => sum + l.monthlyPayment, 0);
  ...
  const calculateProjections = () => {
    ...
    for (let year = 1; year <= projectionYears; year++) {
      ...
      const expenses = (financialData.loanPayments * 12 + financialData.otherExpenses) * yearlyExpenseIncrease;
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            ...
            <div className="bg-white rounded shadow-sm border border-gray-100 p-6">
              ...
              <div className={`text-3xl font-bold mb-1 ${
                financialData.netIncome >= 0 ? 'text-emerald-600' : 'text-red-600'
              }`}>
                ...
                <div className="space-y-3">
                  ...
                  <div className="mt-4 p-4 bg-amber-50 border border-amber-200 rounded-lg">
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-amber-900 font-semibold">Gesamte Restschuld:</span>
                      <span className="text-lg font-bold text-amber-900">
                        {formatCurrency(loanFinancials.reduce((sum, l) => sum + l.remainingBalance, 0))}
                      </span>
                    </div>
                  </div>
                </div>
            ...
            <div className="space-y-6">
              <div>
                ...
                <div className="relative h-12 bg-gray-50 rounded-lg overflow-hidden">
                  ...
                  >
                    {financialData.monthlyRent > 0 && formatCurrency(financialData.monthlyRent)}
                  </div>
                  <div
                  ...
                  >
                    {financialData.loanPayments > 0 && formatCurrency(financialData.loanPayments)}
                  </div>
                </div>
                <div className="flex justify-between mt-2 text-sm">
                  <span className="text-emerald-600 font-semibold">Einnahmen</span>
                  <span className="text-red-600 font-semibold">Ausgaben</span>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-2 px-4 py-2 bg-emerald-100 rounded-lg min-w-[100px]">
                    ...
                    <span className="font-semibold text-emerald-900">{rentIncrease}%</span>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-2 px-4 py-2 bg-red-100 rounded-lg min-w-[100px]">
                    ...
                    <span className="font-semibold text-red-900">{expenseIncrease}%</span>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div>
                ...
                <div className="relative h-64 flex items-end gap-2">
                  {projections.slice(0, 10).map((proj, idx) => {
                    const maxValue = Math.max(...projections.map(p => Math.abs(p.netIncome)));
                    const height = maxValue > 0 ? Math.abs((proj.netIncome / maxValue) * 100) : 0;
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <div className="space-y-2">
                    {projections.slice(0, Math.min(10, projectionYears)).map((proj, idx) => {
                      const maxCumulative = Math.max(...projections.map(p => Math.abs(p.cumulativeIncome)));
                      const width = maxCumulative > 0 ? (Math.abs(proj.cumulativeIncome) / maxCumulative) * 100 : 0;
                ...
                <div>
                  ...
                  {loanFinancials.length > 0 ? (
                    <div className="space-y-2">
                      {projections.slice(0, Math.min(10, projectionYears)).map((proj, idx) => {
                        const totalDebt = loanFinancials.reduce((sum, l) => sum + l.remainingBalance, 0);
                        const yearlyPrincipal = loanFinancials.reduce((sum, l) => sum + (l.monthlyPayment * 12 * 0.6), 0);
                        const estimatedDebt = Math.max(0, totalDebt - (yearlyPrincipal * proj.year));
                        const width = totalDebt > 0 ? (estimatedDebt / totalDebt) * 100 : 0;
          ...
          <div className="mt-6 bg-primary-blue/5 border border-blue-200 rounded-full p-4">
            <div className="flex items-start gap-3">
              ...
              <div className="text-sm text-blue-900">
                ...
                <p>
                  Diese Prognose basiert auf den aktuellen Daten und den eingestellten jährlichen Steigerungsraten.
                  Tatsächliche Ergebnisse können aufgrund von Marktveränderungen, Leerständen, Sonderausgaben und
