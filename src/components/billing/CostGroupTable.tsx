...
import { CostItem, COST_TYPES, ALLOCATION_OPTIONS } from "./OperatingCostWizardStep2";
    ...
    <div className="p-6">
      <div className="overflow-x-auto">
        <table className="w-full">
          ...
          <tbody>
            {group.costItems.map((item, index) => (
              <tr
                key={index}
            ...
            {group.customCostItems.map((item, index) => (
              <tr
                key={`custom-${index}`}
                className="border-b border-gray-100 hover:bg-gray-50 bg-blue-50/30"
              >
                <td className="py-3 px-4">
                  <span className="text-gray-900">{item.cost_type}</span>
                </td>
