...
import { Mail, Phone, Clock, Send, ArrowRight, BookOpen } from "lucide-react";
import { useAuth } from "../contexts/AuthContext";
import { useLanguage } from "../contexts/LanguageContext";
import { Button } from "./ui/Button";

const WHATSAPP_NUMBER = "4915679796826";
  ...
  return (
    <div>
      ...
      <a
        ...
        className="flex items-center gap-4 p-5 bg-white rounded-lg mb-8"
      >
        <div className="w-10 h-10 rounded-full bg-[#EEF4FF] border border-[#DDE7FF] flex items-center justify-center shrink-0">
          <BookOpen className="w-5 h-5 text-gray-900" strokeWidth={1.5} />
        </div>
        <div className="flex-1 min-w-0">
          <h3 className="font-semibold text-gray-900">
            {language === "de" ? "Benutzerhandbuch" : "User Guide"}
          </h3>
          <p className="text-sm text-gray-500 mt-0.5">
            {language === "de"
              ? "Schritt-für-Schritt-Anleitungen und Erklärungen zu allen Funktionen."
              : "Step-by-step guides and explanations for all features."}
          </p>
        </div>
