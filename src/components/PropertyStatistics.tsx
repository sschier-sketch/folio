...
export default function PropertyStatistics({
  ...
  const monthlyLoanPayment = loans.reduce(
    (sum, loan) => sum + Number(loan.monthly_payment),
    0,
  );
  const monthlyPrincipal = loans.reduce(
    (sum, loan) => sum + Number(loan.monthly_principal),
    0,
  );
  ...
  const netEquity =
    Number(property.current_value) - totalRemainingBalance + totalRentReceived;
  const totalInvested = Number(property.purchase_price) - totalLoanAmount;
  const roi =
    totalInvested > 0 ? ((netEquity - totalInvested) / totalInvested) * 100 : 0;
  const rentCoverageRatio =
    monthlyLoanPayment > 0 ? (monthlyRent / monthlyLoanPayment) * 100 : 0;
  const principalCoverage =
    monthlyPrincipal > 0 ? (monthlyRent / monthlyPrincipal) * 100 : 0;
  const loan = loans[0];
  ...
  if (loan) {
    ...
    profitAtLoanEnd =
      netEquity +
      futureRentIncome -
      (Number(property.purchase_price) - totalLoanAmount) -
  ...
  return (
    <div className="space-y-6">
      {" "}
      {loans.length === 0 ? (
        <div className="bg-white rounded shadow-sm p-12 text-center text-gray-300">
          {" "}
          <PiggyBank className="w-16 h-16 mx-auto mb-4 text-gray-200" />{" "}
          <p className="text-lg font-medium">Keine Kredite vorhanden</p>{" "}
          <p className="text-sm mt-2">
            Fügen Sie einen Kredit hinzu, um detaillierte Finanzanalysen zu
            sehen.
          </p>{" "}
        </div>
      ) : (
        <>
            ...
            <div className="mb-4">
              ...
              <div className="flex justify-between text-sm mb-2">
                ...
                <span className="font-semibold text-dark">
                  {calculateLoanProgress().toFixed(1)}%
                </span>{" "}
              </div>{" "}
              ...
              <div className="flex items-center justify-between p-4 bg-red-50 rounded-lg border border-red-200">
                ...
                <div>
                  ...
                  <div className="text-xs text-red-700 mt-2">
                    {" "}
                    Davon Tilgung: {formatCurrency(monthlyPrincipal)} | Zinsen:{" "}
          ...
          <div className="bg-white rounded p-6">
            {" "}
            <h3 className="text-lg font-semibold text-dark mb-4">
              Finanzieren die Mieter die Immobilie?
            </h3>{" "}
            <div className="space-y-4">
              {" "}
              ...
              <div className="p-4 bg-primary-blue/5 rounded-full border border-blue-200">
                ...
                <ul className="space-y-1 text-sm text-primary-blue">
                  {" "}
                  <li>
                    • Monatliche Mieteinnahmen: {formatCurrency(monthlyRent)}
                  </li>{" "}
                  <li>
                    • Monatliche Zinskosten: {formatCurrency(monthlyInterest)}
                  </li>{" "}
                  <li>
                    • Monatliche Tilgung: {formatCurrency(monthlyPrincipal)}
                  </li>{" "}
                  <li className="font-semibold pt-2">
                    ...
                    {rentCoverageRatio >= 100
                      ? "die gesamte Rate"
                      : `${formatPercent(rentCoverageRatio)} der Rate`}{" "}
          ...
          <div className="bg-white rounded p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="p-4 bg-gray-50 rounded-lg">
                {" "}
                <div className="text-sm text-gray-400 mb-1">
                  Gesamte Mieteinnahmen (bisher)
                </div>{" "}
                <div className="text-xl font-bold text-dark">
                  {formatCurrency(totalRentReceived)}
                </div>{" "}
                <div className="text-xs text-gray-300 mt-1">
                  Aus {rentPayments.length} bezahlten Mieten
                </div>{" "}
              </div>{" "}
              <div className="p-4 bg-gray-50 rounded-lg">
                {" "}
                ...
                <div className="text-xs text-gray-300 mt-1">
                  {" "}
                  Von {formatCurrency(Number(property.purchase_price))} auf{" "}
                  {formatCurrency(Number(property.current_value))}{" "}
                </div>{" "}
              </div>{" "}
              <div className="p-4 bg-gray-50 rounded-lg">
                {" "}
              ...
              <div className="p-4 bg-gray-50 rounded-lg">
                ...
                <div className="text-xl font-bold text-dark">
                  {" "}
                  {loan
                    ? `${Math.round((new Date(loan.end_date).getTime() - new Date(loan.start_date).getTime()) / (1000 * 60 * 60 * 24 * 365))} Jahre`
                    : "N/A"}{" "}
                </div>{" "}
                <div className="text-xs text-gray-300 mt-1">
                  {" "}
                  {loan
                    ? `Von ${new Date(loan.start_date).toLocaleDateString("de-DE")} bis ${new Date(loan.end_date).toLocaleDateString("de-DE")}`
                    : ""}{" "}
                </div>{" "}
              </div>{" "}
            </div>{" "}
